...
const { listTemplates, coerceTemplate } = require('../utils/templates');
...
router.get('/:id/preview', (req, res) => {
  ...
  res.send(renderResumeHtml(resume, template));
});
  ...
  try {
    switch (format) {
      case 'pdf': {
        ...
        try {
          const buffer = await htmlToPdf(renderResumeHtml(resume, template));
      ...
      case 'html': {
        ...
        return res.send(renderResumeHtml(resume, template));
      }
