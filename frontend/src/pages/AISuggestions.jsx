...
export default function AISuggestions() {
  ...
  const rewriteBullet = async () => {
    ...
    setError(''); setLoadingBullet(true); setBulletResult(null);
    try {
      const res = await api.post('/ai/rewrite-bullet', { resumeId, bulletText });
      setBulletResult(res.data);
      refreshQuota();
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to rewrite bullet point.');
    } finally {
      ...
      <div className="grid-2">
        ...
        <Card title="⚡ Rewrite a Bullet Point">
          ...
          <button
            ...
            disabled={loadingBullet || !resumeId}
            style={{ marginTop: 10 }}
          >
          ...
          {bulletResult && (
            <div style={{ marginTop: 16 }} className="animate-in">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <button
                  className="btn btn-secondary btn-sm"
                  onClick={() => copyToClipboard(bulletResult.text)}
                >
                  📋 Copy
                </button>
              </div>
