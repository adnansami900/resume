...
const EMPTY_DB = () => ({
  users: [], resumes: [], applications: [], ats_scans: [], ai_suggestions: [],
});
...
function getAllResumes(userId) {
  return data.resumes
    ...
    .map(({ id, title, version, full_name, template, updated_at, created_at }) =>
      ({ id, title, version, full_name, template: template || 'modern', updated_at, created_at }));
}
