...
**Quick start:** from the repo root, `node run.js` (or `run.bat` / `./run.sh`) is the one command for everything — it calls `setup.js` automatically on first run (installs both `backend/` and `frontend/` deps, creates `backend/.env` with a random `JWT_SECRET` pre-filled) then starts both servers; subsequent runs skip straight to starting since `setup.js`'s own checks make it a no-op once everything exists. `setup.js`/`start.js` also work standalone (`setup.bat`/`start.bat`, `./setup.sh`/`./start.sh`) if you want the steps separate — `start.js` refuses to run until setup has been done (checks for `.env` and `node_modules`) rather than failing confusingly mid-boot. Both servers print with colour-coded `[BACKEND]`/`[FRONTEND]` line prefixes and stop together on a single Ctrl+C (full process-tree kill on both platforms, not just the immediate child — `npm start` spawns further children like `react-scripts`). All of this is plain Node (`run.js`/`setup.js`/`start.js`), so it works identically on Windows/Mac/Linux; the `.bat`/`.sh` files are one-line wrappers.
